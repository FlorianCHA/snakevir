...
```{r input file}
#Abundace clust correlation
correlation.clustering <- FALSE
...
#Tools version directry
tools_version_directory <- params$tools_version_directory
# Database path for date creation
diamond_database <- params$diamond_database
blast_database <- params$blast_database

# Path to stat mapping
path.stat <- paste(dirname(params$output_directory),"/stats_run_", project.name, '.csv', sep='')
```
...
| Filter Name        | Seuil                      |
|----------------------|----------------------------|
| Correlation clustering  | ```r correlation.clustering```    |
| Abundance for Clustering  | ```r abundance.clust```    |
...
| File Name            | Path                      |
|----------------------|----------------------------|
| Blast                | ```r basename(blast.file)```         |
| Virus Taxonomy       | ```r basename(taxo.file) ```         |
| Hosts Taxonomy       | ```r basename(host.file)```          |
| Count table          | ```r basename(count.file)```          |
| Coverage             | ```r basename(coversage.stat.file)``` |
| 10X Coverage         | ```r basename(cov10x.file)```          |
...
| File Name            | Path                      |
|----------------------|----------------------------|
| Blast                | ```r basename(output_blast)```         |
| Virus Taxonomy       | ```r basename(output_taxo) ```         |
| OTU blast            | ```r basename(output_otu_stat)```          |
| Count table          | ```r basename(output_count)```          |
| Stat by contig       | ```r basename(output_stat_qseqid)``` |
| Final output         | ```r basename(final_table)```          |

## Snakevir information

### Tool's Version
```{r Tools version}
# bwa version
bwa.file <- paste(tools_version_directory,'/bwa.version',sep = "",collapse = NULL)
bwa.data <- read.csv(bwa.file)
bwa_version = strsplit(bwa.data$Program..bwa..alignment.via.Burrows.Wheeler.transformation.[1],split=' ')[[1]][2]
# Megahit version
Megahit.file <- paste(tools_version_directory,'/megahit.version',sep = "",collapse = NULL)
Megahit.data <- read.csv(Megahit.file,header=FALSE)
Megahit.version <- strsplit(Megahit.data$V1[1],split=' ')[[1]][2]
# Cap3 version
cap3.file <- paste(tools_version_directory,'/cap3.version',sep = "",collapse = NULL)
cap3.data <- read.csv(cap3.file,header=FALSE)
cap3.version <- strsplit(cap3.data$V1[1],split='Size')[[1]][1]
# samtools version
samtools.file <- paste(tools_version_directory,'/samtools.version',sep = "",collapse = NULL)
samtools.data <- read.csv(samtools.file,header=FALSE)
samtools.version <- strsplit(samtools.data$V1[2],split=' ')[[1]][2]
# picard version
picard.file <- paste(tools_version_directory,'/picard.version',sep = "",collapse = NULL)
picard.data <- read.csv(picard.file,header=FALSE)
picard.version <- strsplit(picard.data$V1[1],split=':')[[1]][2]
# diamond version
diamond.file <- paste(tools_version_directory,'/diamond.version',sep = "",collapse = NULL)
diamond.data <- read.csv(diamond.file,header=FALSE)
diamond.version <- strsplit(diamond.data$V1[1],split=' ')[[1]][3]
# flash version
flash.file <- paste(tools_version_directory,'/flash.version',sep = "",collapse = NULL)
flash.data <- read.csv(flash.file,header=FALSE)
flash.version <- strsplit(flash.data$V1[1],split='v')[[1]][2]
# cutadapt version
cutadapt.file <- paste(tools_version_directory,'/cutadapt.version',sep = "",collapse = NULL)
cutadapt.data <- read.csv(cutadapt.file,header=FALSE)
cutadapt.version <- cutadapt.data$V1[1]
# python version
python.file <- paste(tools_version_directory,'/python.version',sep = "",collapse = NULL)
python.data <- read.csv(python.file,header=FALSE)
python.version <- strsplit(python.data$V1[1],split=' ')[[1]][2]
# perl version
perl.file <- paste(tools_version_directory,'/perl.version',sep = "",collapse = NULL)
perl.data <- read.csv(perl.file,header=FALSE)
perl.version <- str_replace(strsplit(perl.data$V3[1],split=' ')[[1]][4],'v','')
```

| Tools                | Version                    |
|----------------------|----------------------------|
| bwa                  | ```r bwa_version```        |
| megahit              | ```r Megahit.version ```   |
| cap3                 | ```r cap3.version```       |
| samtools             | ```r samtools.version```   |
| picard               | ```r picard.version ```    |
| diamond              | ```r diamond.version ```   |
| flash                | ```r flash.version ```     |
| cutadapt             | ```r cutadapt.version ```  |
...
```{r Database version}
# diamond nr database data
list_path_split <- unlist(strsplit(diamond_database, "/"))
basename_diamond <- list_path_split[length(list_path_split)]
diamond_database <- str_sub(diamond_database, end=-(nchar(basename_diamond)+2))
diamond_data <- file.info(diamond_database)$mtime
# blast nt database data
basename_blast <- unlist(strsplit(blast_database, "/"))[length( unlist(strsplit(blast_database, "/")))]
blast_database <- str_sub(blast_database, end=-(nchar(basename_blast)+2))
blast_data = file.info(blast_database)$mtime

```

| Database              | Path                       | Download Date             |
|-----------------------|----------------------------|---------------------------|
| Diamond NR            | ```r diamond_database ```  | ```r diamond_data ```       |
| Blast NT              | ```r blast_database ```    | ```r blast_data ```       |
...
### Reads information by Step

```{r stat run}
library('DT')
data = read.csv(path.stat,row.names=1)
datatable(data, caption =" Table n°1: Run Information",extensions = 'Buttons', options = list(dom = 'Blfrtip',pageLength= 100,buttons = c('copy', 'csv', 'excel', 'pdf', 'print'))) 
```
...
```{r Step.5 Unwanted Species Filter}
...
### Table
```{r Step.9 Table Output}
data <- py$blast_mean
data <- data[1:ncol(data)]
data = data[,!(names(data) %in% c("species_grp"))]
datatable(data, caption =" Table n°2: Count Table",extensions = 'Buttons', options = list(dom = 'Blfrtip',buttons = c('copy', 'csv', 'excel', 'pdf', 'print'))) 
```
